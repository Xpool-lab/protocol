...
export enum eContractid {
  ...
  AaveCollectorImpl = 'AaveCollectorImpl',
  AaveCollector = 'AaveCollector',
  Proxy = 'Proxy',
...
export interface iXpoolAssetBase<T> {
  XPO: T;
  USDT: T;
  DAI: T;
  USDC: T;
...
export type iXPoolAssets<T> = Pick<
  iXpoolAssetBase<T>,
  'XPO' | 'USDT' | 'DAI' | 'USDC' | 'BUSD' | 'BNB' | 'ETH' | 'BTC' | 'DOT' | 'LINK' | 'YFI' | 'CAKE'
>;
...
export enum XPOTokenContractId {
  XPO = 'XPO',
  USDT = 'USDT',
  DAI = 'DAI',
  USDC = 'USDC',
...
export interface IXoolConfiguration extends ICommonConfiguration {
  ReservesConfig: iXPoolAssets<IReserveParams>;
}
