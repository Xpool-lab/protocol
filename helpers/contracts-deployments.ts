...
import {
  ...
  ChainLinkRegistryFactory,
  AirdropFactory,
  AaveCollectorFactory,
  AaveCollector,
} from '../types';
import {
  withSaveAndVerify,
...
export const deployPriceOracle = async (priceAdmin: tEthereumAddress, verify?: boolean) => {
  const args: [tEthereumAddress] = [priceAdmin];
  return withSaveAndVerify(
    // @ts-ignore
    await new PriceOracleFactory(await getFirstSigner()).deploy(...args),
...
export const deployAaveCollector = async (verify?: boolean) =>
  withSaveAndVerify(
    await new AaveCollectorFactory(await getFirstSigner()).deploy(),
    eContractid.AaveCollectorImpl,
    [],
    verify
  );

export const deployChainlinkRegistry = async (verify?: boolean) =>
  withSaveAndVerify(
    await new ChainLinkRegistryFactory(await getFirstSigner()).deploy(),
    eContractid.ChainlinkRegistry,
    [],
    verify
  );

export const deployAirdrop = async (root: tStringTokenSmallUnits, verify?: boolean) =>
  withSaveAndVerify(
    await new AirdropFactory(await getFirstSigner()).deploy(root),
    eContractid.Airdrop,
    [],
    verify
  );
...
export const deployAllMockTokens = async (verify?: boolean) => {
  ...
  for (const tokenSymbol of Object.keys(XPOTokenContractId)) {
    let decimals = '18';
