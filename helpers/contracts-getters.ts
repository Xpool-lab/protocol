...
export const getChainlinkRegistry = async (address?: tEthereumAddress) =>
  await ChainLinkRegistryFactory.connect(
    address ||
      (await getDb().get(`${eContractid.ChainlinkRegistry}.${DRE.network.name}`).value()).address,
    await getFirstSigner()
  );

export const getAirDrop = async (address?: tEthereumAddress) =>
  await AirdropFactory.connect(
    address || (await getDb().get(`${eContractid.Airdrop}.${DRE.network.name}`).value()).address,
    await getFirstSigner()
  );
